document.addEventListener("DOMContentLoaded", () => {
    ...
    btnPrint.addEventListener("click", async () => {
        try {
            ...
            const judulPublikasi = document.getElementById("judulPublikasi").value.trim();
            const publikasikeSelect = document.getElementById("Publikasike");
            const publikasike = publikasikeSelect.value ? 
                publikasikeSelect.options[publikasikeSelect.selectedIndex].text : "";
            ...
            data["judulPublikasi"] = judulPublikasi;
            data["Publikasike"] = publikasike;

            const response = await fetch("5_Form Penilaian Publikasi.docx");
            ...
            const fileName = `Penilaian Kelayakan Publikasi - ${safeNama}.docx`;
    ...
    document.getElementById("btnKirim").addEventListener("click", async () => {
        ...
        const data = {
            ...
            nim: nim,
            scores: scores,
            publikasi: publikasi, // Jenis publikasi
            status: status, // Status publikasi
            namajurnal: namajurnal,  // Nama jurnal/publikasi
            judulPublikasi: judulPublikasi, // Judul publikasi
            publikasike: publikasike, // Publikasi ke-
        ...
        fetch("https://script.google.com/macros/s/AKfycbzWVZv6kD23oekUDw1XdeweUkn_QiYdF7Wzo-LAZHbCFnnOaEipoRWDDYVqQUCya5dIKg/exec", {
            method: "POST",
            body: formBody
        })
            .then(res => res.json())
